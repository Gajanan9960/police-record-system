...
def list_cases():
    ...
    # Restrict Officers to only their assigned cases (Lead or Team Member)
...
def create_case():
    ...
    if form.validate_on_submit():
        ...
        if not case_num:
            # Sequential Case Number Logic: CASE-{StationID}-{Year}-{Sequence}
            now = datetime.now()
            year = now.strftime('%Y')
            
            # Count cases for this station in this year to determine sequence
            # Using LIKE query to find cases starting with CASE-{year}
            # Note: We want unique sequence per Year globally if not station scoped? 
            # User said "CASE-2025-001 in all cases", implying mapped to Year. 
            # However, `station_scoped` logic implies we might want separate sequence per station?
            # Or global sequence? "in all cases" and format CASE-2025-001 suggests global or just ignoring station in ID str.
            # Given we are filtering by `station_scoped(Case.query)` elsewhere, removing station ID from string 
            # might cause collisions if multiple stations exist and use same format.
            # But here we only have 1 active station (ID 1). 
            # I will follow user request strictly: CASE-{Year}-{Sequence}.
            
            prefix = f"CASE-{year}-"
            # If multiple stations, this logic needs global uniqueness check or station filter + suffix collision handling.
            # For now, simplistic approach: Count distinct IDs starting with prefix.
            count = Case.query.filter(Case.case_number.like(f"{prefix}%")).count()
            
            # Sequence is count + 1. Pad with leading zeros (e.g., 0001)
